# 👋 Hi, I'm Mohamed Dewidar - Senior Flutter Developer 💙

Senior Flutter Developer from Nasr City, Egypt 🇪🇬 with over 5 years of experience in Flutter hybrid mobile development and 2 years in native Android. Skilled in building scalable mobile applications integrated with Django REST Framework backends. Experienced in managing end-to-end app deployment on both Google Play Store and Apple App Store.

📍 **Nasr City, Cairo, Egypt** | 📧 **mohamed.dewidar28@gmail.com** | 📱 **+20 1272553815**

🌐 **[Portfolio](https://dewidarr.github.io/Mohamed-Dewidar/)** | 💼 **[LinkedIn](https://www.linkedin.com/in/mohamed-dewidar-23ba90142)** | 🐙 **[GitHub](https://github.com/dewidarr)**

---

## 🚀 Tech Stack & Skills

### 📱 **Mobile Development**
- **Flutter (Dart)** - Cross-platform apps for iOS, Android, and Web
- **Native Integration** - Kotlin/Swift platform channels, Payment Gateways (Stripe, Paypal, PayTabs, Redsys)
- **State Management** - BLoC, Cubit, Provider, Riverpod, GetX
- **Optimization** - Memory management, App start-up time reduction

### 🏗️ **Architecture & Design**
- **Clean Architecture**
- **Design Patterns** (Singleton, Factory, Builder, Observer)
- **SOLID Principles**
- **Modular Architecture**

### 🌐 **Backend & Database**
- **Python (Django REST Framework)**
- **SQL (PostgreSQL, MySQL, SQLite)**
- **NoSQL (Firebase Firestore, Hive)**

### 🛠️ **DevOps & Tools**
- **CI/CD** - GitHub Actions, Bitbucket Pipelines
- **Version Control** - Git, GitHub, Bitbucket, GitLab
- **Project Management** - JIRA, Trello

---

## 💼 Work Experience

### **3I Vision** | Team Lead Flutter Developer | Jul 2024 – Present
📍 Giza, Egypt

- Led a team of Flutter developers in designing, building, and maintaining cross-platform mobile applications.
- Overseeing the entire mobile app lifecycle — from architecture planning to deployment.
- Collaborated with AI engineers to deliver high-performance, intelligent applications like "DataMind" and "3I Vision".
- Implemented ML model response handling (People Detection, License Plate Recognition).

### **Quantum Sit** | Senior Flutter Developer | Jan 2020 – Jul 2024
📍 Nasr City, Egypt

- Developed **Cogens School Management System**, including Parent, Staff, Student, and Driver apps.
- Integrated multiple payment gateways.
- Developed the Ambulance Application for Egypt.
- Worked on Pub projects and Redsys payment integration.

### **Software Company** | Android Developer | Oct 2018 – Sep 2019
📍 Nasr City, Egypt

- Developed native Android applications using Java/Kotlin.
- Collaborated with cross-functional teams to define and ship new features.

---

## 📱 Featured Projects

### 🏢 **Cogens ERP Ecosystem** (Live)
| App | Description | Links |
|-----|-------------|-------|
| **Cogens Parent** | Track attendance, announcements, payments | [App Store](https://apps.apple.com/eg/app/cogens-parent/id1607885469) • [Play Store](https://play.google.com/store/apps/details?id=com.trispects.school.parents) |
| **Cogens Student** | View schedules, grades, attendance | [App Store](https://apps.apple.com/eg/app/cogens-student/id6476619759) • [Play Store](https://play.google.com/store/apps/details?id=com.trispects.school.students) |
| **Cogens Bus Driver** | Real-time GPS tracking | [App Store](https://apps.apple.com/eg/app/cogens-bus-driver/id6479618217) • [Play Store](https://play.google.com/store/apps/details?id=com.trispects.school.bus.driver) |
| **Cogens Staff** | Attendance recording | [App Store](https://apps.apple.com/eg/app/cogens-staff/id1607885245) • [Play Store](https://play.google.com/store/apps/details?id=com.trispects.school.management) |

### 👁️ **3I Vision Projects** (Live)
| App | Description | Links |
|-----|-------------|-------|
| **3I Vision** | AI-powered People Detection & License Plate Recognition | [App Store](https://apps.apple.com/eg/app/3i-vision/id6670479729) • [Play Store](https://play.google.com/store/apps/details?id=com.vision.watcharea) |
| **DataMind Agent** | Intelligent file manager with AI classification | [App Store](https://apps.apple.com/eg/app/datamind-agent/id6754242027) • [Play Store](https://play.google.com/store/apps/details?id=com.datamind.agent) |
| **Adahi** | AI-based health and qualification detection | [Play Store](https://play.google.com/store/apps/details?id=com.adahi.sa) |

---

## 🌍 Languages

- 🇪🇬 **Arabic** – Native
- 🇺🇸 **English** – Fluent

---

## 📫 Contact Me

- 📧 Email: [mohamed.dewidar28@gmail.com](mailto:mohamed.dewidar28@gmail.com)
- 📱 Phone: [+20 1272553815](tel:+201272553815)
- 💼 LinkedIn: [linkedin.com/in/mohamed-dewidar-23ba90142](https://www.linkedin.com/in/mohamed-dewidar-23ba90142)
- 🐙 GitHub: [github.com/dewidarr](https://github.com/dewidarr)
- 🌐 Portfolio: [eslamabid175.github.io](https://dewidarr.github.io/Mohamed-Dewidar/)

---

<div align="center">

### 💼 Open for Full-time Opportunities

[![Portfolio](https://img.shields.io/badge/Portfolio-1e30f3?style=for-the-badge&logo=google-chrome&logoColor=white)](https://dewidarr.github.io/Mohamed-Dewidar/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mohamed-dewidar-23ba90142)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/dewidarr)

</div>
